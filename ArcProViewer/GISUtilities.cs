    ...
    public class GISUtilities
    {
        ...
        public static async Task AddToMapAsync(TreeViewItemModel item, int index)
        {
            await QueuedTask.Run(() =>
            {
                ...
                Uri uri2 = new Uri(@"C:\GISData\National_Datasets-USACE_National_Dams\usace_national_dams.gpkg");
                Uri uri3 = new Uri(@"C:\GISData\National_Datasets-USACE_National_Dams\usace_national_dams.gpkg\main.dams");


                //var connectionPath = new FileGeodatabaseConnectionPath(uri2);

                //DatabaseConnectionProperties props = new DatabaseConnectionProperties(EnterpriseDatabaseType.SQLite);
                //props.Database = @"C:\GISData\National_Datasets-USACE_National_Dams\usace_national_dams.gpkg";

                //Geodatabase geodatabase = new Geodatabase(new DatabaseConnectionFile(new Uri(@"C:\GISData\National_Datasets-USACE_National_Dams\usace_national_dams.gpkg")));
                //CIMSqlQueryDataConnection sqldc = new CIMSqlQueryDataConnection()
                //{
                //    WorkspaceConnectionString = geodatabase.GetConnectionString(),
                //    GeometryType = esriGeometryType.esriGeometryPoint,
                //    OIDFields = "OBJECTID",
                //    SqlQuery = "select * from main.dams",
                //    Dataset = "dams"
                //};


                CIMStandardDataConnection conn = new CIMStandardDataConnection();
                conn.WorkspaceConnectionString = @"DATABASE=C:\GISData\National_Datasets-USACE_National_Dams\usace_national_dams.gpkg";
                ...
                LayerFactory.Instance.CreateLayer<ArcGIS.Desktop.Mapping.FeatureLayer>(p, MapView.Active.Map as ILayerContainerEdit);


                //        using (var database = new Database(props))
                //        {
                //            // Get all QueryDescriptions (these represent feature tables/views)
                //            var query = database.GetQueryDescription("dams");//.GetQueryDescriptions();

                //            //foreach (var query in queries)
                //            //{
                //            //if (query.HasGeometry()) // only consider spatial tables
                //            //{
                //            //    using (var rowCursor = database.OpenQueryTable(query).Search())
                //            //    {
                //            // Get the extent of this feature class
                //            using (var fc = database.OpenTable(query) as FeatureClass)
                //            {
                //                if (fc != null)
                //                {
                ...
                //                    if (extent != null)
                //                    {
                //                        fullExtent = fullExtent == null ? extent : fullExtent.Union(extent) as Envelope;
                //                    }

                //                    LayerCreationParams p = new LayerCreationParams()
                ...
                //        //}
                //    }
                //LayerFactory.Instance.CreateLayer(uri3, MapView.Active.Map, 0, "test");
                //});
